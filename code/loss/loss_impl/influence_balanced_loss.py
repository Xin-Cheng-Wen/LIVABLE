...
class InfluenceBalancedLoss(CrossEntropy):
    ...
    def __init__(self, para_dict=None):
        ...
        ib_weight = ib_weight / np.sum(ib_weight) * self.num_classes            # 难道不是应该乘上1/num_classes吗？
    ...
    # def forward(self, inputs, targets, feature):
    #     """
    #     Args:
    ...
    #     grads = torch.sum(torch.abs(inputs - F.one_hot(targets, self.num_classes)), 1)
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        feature = torch.sum(torch.abs(kwargs['feature']), 1).reshape(-1, 1)      # 最后一层全连接层的输入
        # grads = torch.sum(torch.abs(F.softmax(inputs, dim=1) - F.one_hot(targets, self.num_classes)), 1)
        grads = torch.sum(torch.abs(inputs - F.one_hot(targets, self.num_classes)), 1)
        ib = grads * feature.reshape(-1)
        ib = self.alpha / (ib + 1e-3)
