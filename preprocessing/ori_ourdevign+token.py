...
def combine(x_node, y_node):
    ...
    if x_type == 'ExpressionStatement':
        if y_type == 'AssignmentExpression':  # type1: ExpressionStatement -> AssignmentExpression
            return True
        if y_type == 'UnaryExpression' and x_node['code'].strip() == y_node['code'].strip():  # type1 ++a
            return True
        if y_type == 'PostIncDecOperationExpression' and x_node['code'].strip() == y_node['code'].strip():  # type1 a++
            return True
        if y_type == 'CallExpression':   # type1: ExpressionStatement -> CallExpression
            return True
    
    if x_type == 'IdentifierDeclStatement' and y_type == 'IdentifierDecl': #type2
        return True
    
    if x_type == 'CallExpression' and y_type == 'ArgumentList':  # type3
        return True
    if x_type == 'Callee' and y_type == 'Identifier':  #type3
        return True
    if x_type == 'Argument' and x_node['code'].strip() == y_node['code'].strip():  #type3 name -> name ; 0 -> 0; 'str' -> 'str'
        return True
    if x_type == 'Condition' and x_node['code'].strip() == y_node['code'].strip():  #type4
        return True
    if x_type == 'ForInit':  #type5 for语句
...
def ast_prune(nodes, ast_edges, ast_id):
    ...
    for item in ast_edges:
        ...
        for y in item['end']:
            ...
            if combine(x_node, y_node) == True:   #当前的头、尾节点可以合并，边不存在
                t_node.remove(y_node_key)
                flag = False
                for sublist in duplist:  #判断头节点是否可以向前合并
                    if x_node_key in sublist:
                        sublist.append(y_node_key)
                        p_node = sublist[0]  # cyp  头尾节点可以合并且头节点可以向前合并
                        flag = True
                        break
                if not flag:  #头节点不能向前合并时
                    repeat.append(y_node_key)
            else:  #当前头尾节点不能合并，边存在，判断当前头节点是否已经向前合并
        ...
        elif p_node == x_node_key and len(t_node) > 0:  #当前头节点未向前合并
            ast_new_edges.append(edges)
        elif p_node != x_node_key:  #当前头节点向前合并
            find = False
...
def get_ncs_edges(all_ast_edges, nsc_type, nodes):
    ...
    for ast_sent in all_ast_edges:
        ...
        nsc_nodes = []  #记录一个句子内的ast节点
        if s_nodes == t_nodes and len(s_nodes) == 1:  #break; continue; return; 特殊情况
            nsc_nodes = t_nodes
...
def var_combine(ast_edges, same_var):
    ...
    pass
# 原cpg + step1 : 所有ast边  -> (ast压缩 ) -> (变量合并) -> nsc -> 其他类型边  -> 边转换 -> self_loop
def graphGeneration(nodes, edges, edge_type_map, ver_edge_type_map):
    ...
    all_ast_edges = []
    all_edges = list()
    #获得除entry和exit节点之外的每个语句节点的ast    所有ast边
    for node in nodes:
        if node['isCFGNode'].strip() != 'True' or node['key'].strip() == 'File':
            continue
        all_nodes.add(node['key'])  #此时all_nodes里是所有节点级别语句
        # entry  exit
        ...
        all_ast_edges.append(ast_edges)  #每个语句的ast边list
    #未压缩前的ast边数 节点数
    #''' 6.15 cyp-
    edge_count_before = 0
    ...
    #''' 6.15 cyp-
    #ast压缩  all_ast_edges -> step1_ast_edges
    '''
    step1_ast_edges = []
    for i in all_ast_edges:    #对每一个句子，删除重复的AST节点，构造新边
    ...
    #添加所有的ast边
    for item in all_ast_edges:
        # break; continue; return; 防止添加self-loop
        if len(item) == 1 and len(item[0]['end']) == 1 and item[0]['start'] == item[0]['end'][0]:
            continue
        for x in item:
            start = x['start']
            for end in x['end']:
                all_edges.append([start, ast_type, end])  #所有语句的ast边集合
    #其他类型边
    for e in edges:
        ...
        if eType != 'IS_FILE_OF' and eType != ast_type:
            if not eType in edge_type_map: #or not start in all_nodes or not end in all_nodes:
                continue
            all_edges.append([start, eType, end])  #除ast边外，增加语句节点之间的边
            #记录句子顺序
    #获得所有的节点
    for e in all_edges:
        start, _, end = e
        all_nodes.add(start)
        all_nodes.add(end)
    if len(all_nodes) == 0 or len(all_nodes) > 400:  # V1是750
...
def word2vec_seq(seq, wv):
    fnrp = list()
    tokens = my_tokenizer(seq)
    i = 0
    for token in tokens:
        if i > 512:
            break
        ...
        nrp = np.add(nrp, embedding)
        if i > 0:
            fnrp = np.concatenate((fnrp, nrp), axis = 0)
            i = i + 1
        else:
            fnrp = nrp
            i =i +1
...
def visual():
    ...
    file_path = "dataset/reveal_parsed"
...
def main():
    ...
    parser.add_argument('--csv', help='normalized csv files to process', default=['./top25_xc/DiverseVul_train/', './top25_xc/DiverseVul_test/', './top25_xc/DiverseVul_valid/'])  #nodes  edges
    # parser.add_argument('--src', help='source c files to process', default='./test_output_c/')
    parser.add_argument('--json_files', help = 'train and test and valid', default=['./train_output.jsonl', 
    './test_output.jsonl', './valid_output.jsonl'])
    parser.add_argument('--wv', default='diverse-wv_models/diverse-train_subtoken_data')  # word2vec模型
    parser.add_argument('--output_dir', default='./our_word2vec_diverse')
    ...
    with jsonlines.open(valid_path, 'r') as reader:
        for idx, line in enumerate(reader):
    ...
    print("*"*100)
    train_output_path = open(os.path.join(output_dir, 'diverse-train-v0.json'), 'w')
    test_output_path = open(os.path.join(output_dir, 'diverse-test-v0.json'), 'w')
    valid_output_path = open(os.path.join(output_dir, 'diverse-valid-v0.json'), 'w')
    output_files = [train_output_path, test_output_path, valid_output_path]

    train_num_path = open(os.path.join(output_dir, 'diverse-train-num-v0.json'), 'w')
    test_num_path = open(os.path.join(output_dir, 'diverse-test-num-v0.json'), 'w')
    valid_num_path = open(os.path.join(output_dir, 'diverse-valid-num-v0.json'), 'w')
    num_files = [train_num_path, test_num_path, valid_num_path]

    train_file = open(os.path.join(output_dir, 'diverse-train-file.json'), 'w')
    test_file = open(os.path.join(output_dir, 'diverse-test-file.json'), 'w')
    valid_file = open(os.path.join(output_dir, 'diverse-valid-file.json'), 'w')
    ...
    #for i in range(2, 3):
        ...
        files = []
        num = 0
        for j, entry in enumerate(tqdm(data[i])):
            file_name = entry['file_path']
            
            #source_tokens = [tokenizer.cls_token] + code_tokens + [tokenizer.sep_token]
            
            seq = word2vec_seq(entry['func'], model)

            #print(source_tokens)
            #print(source_ids)
            #code_1 = tokenizer(entry['code'],padding='max_length', truncation=True,return_tensors='pt')
            #fnrp = code_1["input_ids"]
            #fnrp = fnrp.view(-1)
            nodes_path = os.path.join(args.csv[i], file_name, 'tmp/' + file_name+'/nodes.csv')
            print(nodes_path)
            # exit(0)
            edges_path = os.path.join(args.csv[i], file_name, 'tmp/' + file_name+'/edges.csv')
